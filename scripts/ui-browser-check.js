...
try{
  for(const view of ['docs','kanban','planner']){
    ...
    const dom=await run([...common,'--dump-dom',`http://127.0.0.1:4173/test/visual/fixture.html?view=${view}`]);
    if(!dom.includes('data-ui-check="pass"'))throw new Error(`Layout contract did not pass for ${view}; check may be absent or overflow is present.`);
    if(!dom.includes('data-a11y-check="pass"'))throw new Error(`Accessibility fixture contract failed for ${view}`);
  }
  const reduced=await run([...common,'--force-prefers-reduced-motion','--dump-dom','http://127.0.0.1:4173/test/visual/fixture.html?view=planner']);if(!reduced.includes('data-ui-check="pass"'))throw new Error('Reduced-motion browser contract failed.');
  const contrast=await run([...common,'--force-high-contrast','--dump-dom','http://127.0.0.1:4173/test/visual/fixture.html?view=kanban']);if(!contrast.includes('data-ui-check="pass"')||!contrast.includes('data-a11y-check="pass"'))throw new Error('High-contrast browser contract failed.');
