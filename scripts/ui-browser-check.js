...
try {
  for(const view of ['docs','kanban','planner']){
    for(const width of [390,768,1440]){
      ...
      await run(['--headless=new','--no-sandbox','--disable-gpu','--hide-scrollbars',`--window-size=${width},900`,`--screenshot=${shot}`,url]);
      actual[key]=hash(await readFile(shot)); console.log(`VISUAL_HASH ${key} ${actual[key]}`);
      if(expected[key]&&expected[key]!==actual[key]) throw new Error(`Visual regression: ${key} expected ${expected[key]} got ${actual[key]}`);
    }
    const dom=await run(['--headless=new','--no-sandbox','--disable-gpu','--dump-dom',`http://127.0.0.1:4173/test/visual/fixture.html?view=${view}`]);
  ...
  const reduced=await run(['--headless=new','--no-sandbox','--disable-gpu','--force-prefers-reduced-motion','--dump-dom','http://127.0.0.1:4173/test/visual/fixture.html?view=planner']);
  if(!reduced.includes('data-ui-check="pass"')) throw new Error('Reduced-motion browser contract failed.');
  const contrast=await run(['--headless=new','--no-sandbox','--disable-gpu','--force-high-contrast','--dump-dom','http://127.0.0.1:4173/test/visual/fixture.html?view=kanban']);
  if(!contrast.includes('data-ui-check="pass"')||!contrast.includes('data-a11y-check="pass"')) throw new Error('High-contrast browser contract failed.');
