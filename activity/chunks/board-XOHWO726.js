import{a as xe}from"./chunk-7OSQWNIS.js";import"./chunk-EPXOE4VI.js";var ae=Object.freeze([{id:"backlog",label:"Backlog",color:"#8a8e9b"},{id:"todo",label:"Por hacer",color:"#5865f2"},{id:"doing",label:"En curso",color:"#f0b232"},{id:"done",label:"Hecho",color:"#23a55a"}]);var ue=Object.freeze([{id:"low",label:"Baja",color:"#8a8e9b",order:1},{id:"medium",label:"Media",color:"#5865f2",order:2},{id:"high",label:"Alta",color:"#f0b232",order:3},{id:"urgent",label:"Urgente",color:"#f23f43",order:4}]),Ve=new Set(ae.map(t=>t.id)),Ze=new Set(ue.map(t=>t.id)),ea=Object.freeze({in_progress:"doing","in-progress":"doing"});var O=Object.freeze([{id:"blurple",name:"Azul",color:"#5865f2"},{id:"emerald",name:"Verde",color:"#23a55a"},{id:"amber",name:"\xC1mbar",color:"#f0b232"},{id:"crimson",name:"Rojo",color:"#f23f43"},{id:"purple",name:"P\xFArpura",color:"#9b59b6"},{id:"pink",name:"Rosa",color:"#eb459e"},{id:"cyan",name:"Cian",color:"#00b0f4"},{id:"slate",name:"Gris",color:"#8a8e9b"}]);function W(t){if(!t)return O[0].color;let r=0;for(let e=0;e<t.length;e+=1)r=(r<<5)-r+t.charCodeAt(e),r|=0;return O[Math.abs(r)%O.length].color}function ie(t){if(!t)return[];if(Array.isArray(t)){let a=new Set,n=[];for(let d of t){if(!d)continue;let l=typeof d=="string"?d.trim():String(d.name||"").trim();if(!l)continue;let w=l.toLocaleLowerCase("es");if(a.has(w))continue;a.add(w);let m=typeof d=="object"&&d.color?d.color:W(l);if(n.push({name:l.slice(0,24),color:m}),n.length>=8)break}return n}let r=new Set,e=[];for(let a of String(t).split(",")){let n=a.trim().replace(/\s+/g," ").slice(0,24),d=n.toLocaleLowerCase("es");if(!(!n||r.has(d))&&(r.add(d),e.push({name:n,color:W(n)}),e.length>=8))break}return e}var Be="1539704001535156254",ve="bardo:board:",ke="board:";var ye={urgent:{label:"Urgente",color:"#f23f43",bg:"rgba(242, 63, 67, 0.15)",icon:'<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M8.5 14.5A2.5 2.5 0 0 0 11 12c0-1.38-.5-2-1-3-1.072-2.143-.224-4.054 2-6 .5 2.5 2 4.9 4 6.5 2 1.6 3 3.5 3 5.5a7 7 0 1 1-14 0c0-1.153.433-2.294 1-3a2.5 2.5 0 0 0 2.5 2.5z"/></svg>'},high:{label:"Alta",color:"#f0b232",bg:"rgba(240, 178, 50, 0.15)",icon:'<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="18 15 12 9 6 15"/></svg>'},medium:{label:"Media",color:"#5865f2",bg:"rgba(88, 101, 242, 0.15)",icon:'<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="5" fill="currentColor"/></svg>'},low:{label:"Baja",color:"#8a8e9b",bg:"rgba(138, 142, 155, 0.15)",icon:'<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="6 9 12 15 18 9"/></svg>'}},J=null,Q=null,_=null,x=null,z=null,X=[],G=[],te=[],H=null,me=!1,be=null,ge=null,$={search:"",onlyMyTasks:!1,assignee:"all",priority:"all",label:"all"},de=null,j=[];function je(){return(window.location.hostname||"").match(/^([a-zA-Z0-9_-]+)\.discordsays\.com$/i)?.[1]||Be}function fe(t){let r=String(t||"").trim();return r.startsWith(ve)?r.slice(ve.length)||null:r.startsWith(ke)&&r.slice(ke.length)||null}function se(t){let r=t==="light";document.documentElement.setAttribute("data-theme",r?"light":"dark"),document.documentElement.classList.toggle("theme-light",r),document.documentElement.classList.toggle("theme-dark",!r)}function we(t){let e=new URLSearchParams(window.location.search).get("theme");if(e?se(e):t?.theme?se(t.theme):t?.config?.theme?se(t.config.theme):window.matchMedia&&window.matchMedia("(prefers-color-scheme: light)").matches?se("light"):se("dark"),t?.subscribe)try{t.subscribe("THEME_CHANGE",({theme:a})=>{a&&se(a)})}catch{}}async function Ce(t){if(t){try{if(t.commands?.getInstanceConnectedParticipants){let r=await t.commands.getInstanceConnectedParticipants();Array.isArray(r?.participants)&&r.participants.length>0&&(X=r.participants,!z&&X.length>0&&(z=X[0]))}}catch(r){console.warn("No se pudieron obtener los participantes de Discord:",r)}try{if(t.commands?.getChannel&&t.channelId){let r=await t.commands.getChannel({channel_id:t.channelId});if(Array.isArray(r?.recipients))for(let e of r.recipients)e?.id&&!X.some(a=>String(a.id)===String(e.id))&&X.push(e)}}catch{}}}async function Ue(){if(J)return J;let r=new URLSearchParams(window.location.search).has("frame_id")||window.location.hostname.endsWith(".discordsays.com");if(we(null),!r)return null;try{let e=await globalThis.__bardoActivityAuth?.ready,a=e?.sdk||new xe(je());e?.sdk||await a.ready(),J=a,we(a);try{a.subscribe&&a.subscribe("ACTIVITY_INSTANCE_PARTICIPANTS_UPDATE",({participants:n})=>{Array.isArray(n)&&(X=n,!z&&X.length>0&&(z=X[0]))})}catch{}return await Ce(a),a}catch(e){return console.warn("No se pudo iniciar DiscordSDK para el tablero:",e),null}}async function _e(t,r=5){if(!t)return null;for(let e=0;e<r;e+=1){let a=await fetch(`/api/activity-context/${encodeURIComponent(t)}`,{cache:"no-store",headers:{Accept:"application/json"}}).catch(()=>null);if(a?.ok)return a.json();e<r-1&&await new Promise(n=>setTimeout(n,Math.min(180*2**e,1200)))}return null}async function De(){let t=new URLSearchParams(window.location.search),r=await Ue();_=(await globalThis.__bardoActivityAuth?.ready)?.instanceId||r?.instanceId||t.get("instance_id")||null;let a=fe(r?.customId)||fe(t.get("custom_id"))||t.get("board");if(a)return a;let n=await _e(_);return fe(n?.documentId)}function s(t){return String(t||"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function ce(t){return String(t||"?").split(/\s+/).filter(Boolean).slice(0,2).map(r=>r[0]?.toUpperCase()).join("")||"?"}function N(t,r="info"){let e=document.querySelector("#bardo-toast");e&&e.remove(),ge&&clearTimeout(ge);let a=document.createElement("div");a.id="bardo-toast",a.className=`kanban-toast toast-${r}`,a.innerHTML=`
    <span class="toast-icon">${r==="success"?"\u2713":r==="error"?"\u2715":"\u2139"}</span>
    <span class="toast-msg">${s(t)}</span>
  `,document.body.appendChild(a),requestAnimationFrame(()=>a.classList.add("is-visible")),ge=setTimeout(()=>{a.classList.remove("is-visible"),setTimeout(()=>a.remove(),250)},3200)}function Re(){if(document.querySelector("#bardo-kanban-styles"))return;let t=document.createElement("style");t.id="bardo-kanban-styles",t.textContent=`
  ...
  `,document.head.appendChild(t)}function Pe(){Re(),document.documentElement.dataset.bardoMode="board",document.querySelector("#bardo-kanban")?.remove();let t=document.createElement("main");t.id="bardo-kanban",t.className="kanban-shell";let r=document.querySelector(".brand-avatar")?.src||"";return t.innerHTML=`
    <header class="kanban-topbar">
      <div class="kanban-brand-group">
        ${r?`
          <div class="kanban-avatar-box">
            <img class="kanban-avatar" src="${s(r)}" alt="Bardo" />
          </div>
        `:""}
  ...
  `,document.body.appendChild(t),t.querySelector("#btn-sync")?.addEventListener("click",async()=>{await V(!0)}),t.querySelector("#btn-new-task-global")?.addEventListener("click",()=>{let e=(x?.columns||ae)[0]?.id||"backlog";pe({mode:"create",status:e})}),t.querySelector("#kanban-content")}function Ae(t=[]){let r=new Map;for(let e of t){let a=ie(e.labels||[]);for(let n of a){if(!n.name)continue;let d=n.name.toLowerCase();r.has(d)||r.set(d,{name:n.name,color:n.color||W(n.name)})}}return Array.from(r.values())}function Ee(t){let r=new Map;if(Array.isArray(t?.members))for(let e of t.members){if(!e)continue;let a=String(e.id||e.name||e.username);r.set(a,{id:a,name:e.name||e.username||"Usuario",username:e.username||"",avatarUrl:e.avatarUrl||null,roles:Array.isArray(e.roles)?e.roles:[]})}for(let e of G){if(!e||!e.id)continue;let a=String(e.id);r.has(a)||r.set(a,{id:a,name:e.name||e.username||"Usuario",username:e.username||"",avatarUrl:e.avatarUrl||null,roles:Array.isArray(e.roles)?e.roles:[]})}for(let e of X){if(!e||!e.id)continue;let a=String(e.id);r.get(a)||r.set(a,{id:a,name:e.global_name||e.username||"Usuario",username:e.username||"",avatarUrl:e.avatar?`https://cdn.discordapp.com/avatars/${e.id}/${e.avatar}.png?size=64`:null,roles:[]})}if(Array.isArray(t?.tasks)){for(let e of t.tasks)if(e.assigneeName){let a=e.assigneeId?String(e.assigneeId):e.assigneeName;r.has(a)||r.set(a,{id:a,name:e.assigneeName,username:"",avatarUrl:null,roles:[]})}}return Array.from(r.values())}function Ie(t=[]){let r=new Map;for(let e of G)!e||!e.id||r.set(String(e.id),{id:String(e.id),name:e.name||e.username||"Usuario",username:e.username||"",avatarUrl:e.avatarUrl||null});if(Array.isArray(x?.members))for(let e of x.members){if(!e)continue;let a=String(e.id||e.name||e.username),n=e.name||e.username||"Usuario",d=r.get(a);r.set(a,{id:a,name:n,username:e.username||d?.username||"",avatarUrl:e.avatarUrl||d?.avatarUrl||null})}for(let e of X){if(!e.id)continue;let a=e.nickname||e.global_name||e.username||"Usuario",n=r.get(String(e.id));r.set(String(e.id),{id:String(e.id),name:a,username:e.username||n?.username||"",avatarUrl:n?.avatarUrl||(e.avatar?`https://cdn.discordapp.com/avatars/${e.id}/${e.avatar}.png?size=64`:null)})}if(z?.id){let e=z.global_name||z.username||"Yo",a=r.get(String(z.id));r.set(String(z.id),{id:String(z.id),name:e,username:z.username||a?.username||"",avatarUrl:a?.avatarUrl||(z.avatar?`https://cdn.discordapp.com/avatars/${z.id}/${z.avatar}.png?size=64`:null)})}for(let e of t)if(e.assigneeId&&e.assigneeName){let a=String(e.assigneeId);r.has(a)||r.set(a,{id:a,name:e.assigneeName,username:"",avatarUrl:null})}return Array.from(r.values())}function Oe(t){let r=new Map;if(t){if(Array.isArray(t.members))for(let e of t.members){if(!e)continue;let a=String(e.id||e.name||e.username),n=e.name||e.username||"Usuario";r.set(a,{id:a,name:n,username:e.username||""})}if(Array.isArray(t.tasks)){for(let e of t.tasks)if(e.assigneeName){let a=e.assigneeId?String(e.assigneeId):e.assigneeName;r.has(a)||r.set(a,{id:a,name:e.assigneeName,username:""})}}}return Array.from(r.values())}function Te(){let t=new Set,r=new Set;z?.id&&t.add(String(z.id));let e=localStorage.getItem("bardo_my_user_id");e&&t.add(String(e)),z?.username&&r.add(z.username.toLowerCase().replace(/^@/,"")),z?.global_name&&r.add(z.global_name.toLowerCase());let a=localStorage.getItem("bardo_my_user_name");return a&&r.add(a.toLowerCase().replace(/^@/,"")),{ids:Array.from(t),names:Array.from(r)}}function He(t=[]){return t.filter(r=>{if($.search){let e=$.search.toLowerCase(),a=(r.title||"").toLowerCase().includes(e),n=(r.description||"").toLowerCase().includes(e),d=(r.assigneeName||"").toLowerCase().includes(e),w=ie(r.labels||[]).some(m=>(m.name||"").toLowerCase().includes(e));if(!a&&!n&&!d&&!w)return!1}if($.assignee!=="all")if($.assignee==="unassigned"){if(r.assigneeId||r.assigneeName)return!1}else{let e=$.assignee.toLowerCase(),a=String(r.assigneeId||"").toLowerCase(),n=String(r.assigneeName||"").toLowerCase().replace(/^@/,"");if(a!==e&&n!==e&&!n.includes(e)&&!e.includes(n))return!1}if($.onlyMyTasks){let{ids:e,names:a}=Te(),n=r.assigneeId?String(r.assigneeId):"",d=(r.assigneeName||"").toLowerCase().replace(/^@/,""),l=!1;if(n&&e.includes(n))l=!0;else if(d&&a.length>0){for(let w of a)if(d===w||d.includes(w)||w.includes(d)){l=!0;break}}if(!l)return!1}return!($.priority!=="all"&&r.priority!==$.priority||$.label!=="all"&&!ie(r.labels||[]).some(n=>(n.name||"").toLowerCase()===$.label.toLowerCase()))})}function Fe(t){let r=ye[t.priority]||ye.medium,a=ie(t.labels||[]).map(d=>{let l=d.color||W(d.name);return`
      <span class="task-chip" style="background: ${l}20; border: 1px solid ${l}45; color: ${l};">
        ${s(d.name)}
      </span>
    `}).join(""),n=t.assigneeName?`<div class="task-assignee"><span class="task-assignee-avatar">${s(ce(t.assigneeName))}</span><span class="task-assignee-name">${s(t.assigneeName)}</span></div>`:'<div class="task-assignee"><span class="task-assignee-avatar">\u2014</span><span class="task-assignee-name">Sin asignar</span></div>';return`
    <article class="task-card" draggable="true" data-task-id="${s(t.id)}" tabindex="0" role="button" aria-label="Ver o editar ${s(t.title)}">
      <div class="task-header">
        <span class="priority-badge" style="color: ${r.color}; background: ${r.bg};">
          ${r.icon} ${r.label}
        </span>
      </div>
      <h3 class="task-title">${s(t.title)}</h3>
      ${t.description?`<p class="task-description">${s(t.description)}</p>`:""}
      ${a?`<div class="task-labels">${a}</div>`:""}
      <footer class="task-footer">
        ${n}
      </footer>
    </article>
  `}function R(t,r){x=r,document.title=`${r.name} \xB7 Bardo Kanban`;let e=Array.isArray(r.columns)&&r.columns.length>0?r.columns:ae,a=r.tasks||[],n=He(a),d=Ae(a),l=Oe(r),w=e[0]?.id||"backlog",m=Object.fromEntries(e.map(u=>[u.id,[]])),v=Object.fromEntries(e.map(u=>[u.id,0]));for(let u of a){let k=m[u.status]?u.status:w;v[k]=(v[k]||0)+1}for(let u of n){let k=m[u.status]?u.status:w;m[k].push(u)}let L=!!($.search||$.onlyMyTasks||$.assignee!=="all"||$.priority!=="all"||$.label!=="all");t.className="",t.innerHTML=`
    <header class="kanban-header">
      <div class="kanban-header-info">
        <p class="kanban-eyebrow">Tablero de equipo</p>
        <div class="kanban-title-wrap">
          <h1 class="kanban-title">${s(r.name)}</h1>
        ...
        ${r.description?`<p class="kanban-description">${s(r.description)}</p>`:""}
      </div>
    </header>
    ...
    <nav class="mobile-column-tabs" aria-label="Pesta\xF1as de columnas">
      ${e.map(u=>{let k=u.color||"#5865f2",E=v[u.id]||0;return`
    ...
    <section class="kanban-scroll-container" aria-label="Columnas Kanban">
      <div class="kanban-track">
        ${e.map(u=>{let k=u.color||"#5865f2",E=m[u.id]?.length||0,T=v[u.id]||0,B=L&&E!==T?`${E}/${T}`:E;return`
  ...
  `,Ye(t)}function Ke(t){let r=document.createElement("div");r.id="bardo-identify-modal-backdrop",r.className="kanban-modal-backdrop",r.innerHTML=`
    <div class="kanban-modal" role="dialog" aria-modal="true" style="max-width: 440px;">
      ...
      <div class="modal-form">
        ...
        <div style="display: flex; flex-direction: column; gap: 4px; max-height: 240px; overflow-y: auto;">
          ${t.map(a=>`
            <button type="button" class="member-menu-item btn-choose-me" data-member-id="${s(a.id)}" data-member-name="${s(a.name)}" data-member-username="${s(a.username||"")}">
              ${a.avatarUrl?`<img src="${s(a.avatarUrl)}" alt="${s(a.name)}" style="width: 26px; height: 26px; border-radius: 50%; object-fit: cover; flex-shrink: 0;" />`:`<span class="member-avatar-mini">${s(ce(a.name))}</span>`}
              <div class="member-info-col">
                <span class="member-name-text" style="font-size: 13px;">${s(a.name)}</span>
                ${a.username?`<span class="member-handle-text">@${s(a.username)}</span>`:""}
              </div>
            </button>
          `).join("")}
        </div>
  ...
  `,document.body.appendChild(r);let e=()=>r.remove();r.querySelector("#btn-identify-close")?.addEventListener("click",e),r.addEventListener("click",a=>{a.target===r&&e()}),r.querySelectorAll(".btn-choose-me").forEach(a=>{a.addEventListener("click",()=>{let n=a.dataset.memberId,d=a.dataset.memberName,l=a.dataset.memberUsername;n&&localStorage.setItem("bardo_my_user_id",n),d&&localStorage.setItem("bardo_my_user_name",d),z={id:n,name:d,username:l,global_name:d},$.onlyMyTasks=!0,e();let w=document.querySelector("#kanban-content")||document.querySelector("#kanban-view");w&&x&&R(w,x),N(`Identificado como ${d}`,"success")})})}function Ye(t){let r=Array.isArray(x?.columns)&&x.columns.length>0?x.columns:ae;t.querySelector("#btn-edit-board")?.addEventListener("click",()=>{Ge(x)}),t.querySelector("#filter-search")?.addEventListener("input",a=>{$.search=a.target.value,R(t,x);let n=t.querySelector("#filter-search");n&&(n.focus(),n.setSelectionRange(n.value.length,n.value.length))}),t.querySelector("#btn-clear-search")?.addEventListener("click",()=>{$.search="",R(t,x)}),t.querySelector("#toggle-my-tasks")?.addEventListener("click",()=>{let{ids:a,names:n}=Te();if(!$.onlyMyTasks&&a.length===0&&n.length===0){let d=Ee(x);if(d.length>0){Ke(d);return}}$.onlyMyTasks=!$.onlyMyTasks,R(t,x)}),t.querySelector("#filter-assignee")?.addEventListener("change",a=>{$.assignee=a.target.value,R(t,x)}),t.querySelector("#filter-priority")?.addEventListener("change",a=>{$.priority=a.target.value,R(t,x)}),t.querySelector("#filter-label")?.addEventListener("change",a=>{$.label=a.target.value,R(t,x)}),t.querySelector("#btn-clear-all-filters")?.addEventListener("click",()=>{$={search:"",onlyMyTasks:!1,assignee:"all",priority:"all",label:"all"},R(t,x)}),t.querySelectorAll("[data-jump-to-status]").forEach(a=>{a.addEventListener("click",()=>{let n=a.dataset.jumpToStatus;t.querySelector(`#col-${n}`)?.scrollIntoView({behavior:"smooth",inline:"center",block:"nearest"}),t.querySelectorAll(".mobile-tab-btn").forEach(l=>l.classList.remove("is-active")),a.classList.add("is-active")})}),t.querySelectorAll("[data-add-to-status]").forEach(a=>{a.addEventListener("click",n=>{n.stopPropagation(),pe({mode:"create",status:a.dataset.addToStatus})})}),t.querySelectorAll("[data-edit-column]").forEach(a=>{let n=a.dataset.editColumn,d=r.find(l=>l.id===n);d&&(a.addEventListener("click",l=>{l.stopPropagation(),Se(d)}),a.addEventListener("keydown",l=>{(l.key==="Enter"||l.key===" ")&&(l.preventDefault(),Se(d))}))}),t.querySelectorAll(".task-card").forEach(a=>{let n=null,d=!1;a.addEventListener("click",()=>{if(d)return;let l=a.dataset.taskId,w=(x?.tasks||[]).find(m=>m.id===l);w&&pe({mode:"edit",task:w})}),a.addEventListener("keydown",l=>{if(l.key==="Enter"||l.key===" "){l.preventDefault();let w=a.dataset.taskId,m=(x?.tasks||[]).find(v=>v.id===w);m&&pe({mode:"edit",task:m})}}),a.addEventListener("dragstart",()=>{H=a.dataset.taskId,a.classList.add("is-moving")}),a.addEventListener("dragend",()=>{H=null,a.classList.remove("is-moving"),t.querySelectorAll(".kanban-column").forEach(l=>l.classList.remove("is-over"))}),a.addEventListener("touchstart",()=>{d=!1,n=setTimeout(()=>{H=a.dataset.taskId,a.classList.add("is-touch-dragging"),navigator.vibrate&&navigator.vibrate(30)},220)},{passive:!0}),a.addEventListener("touchmove",l=>{if(!H){clearTimeout(n);return}d=!0;let w=l.touches[0],v=document.elementFromPoint(w.clientX,w.clientY)?.closest(".kanban-column");t.querySelectorAll(".kanban-column").forEach(L=>L.classList.remove("is-over")),v&&v.classList.add("is-over")},{passive:!0}),a.addEventListener("touchend",async l=>{if(clearTimeout(n),H){let w=l.changedTouches[0],v=document.elementFromPoint(w.clientX,w.clientY)?.closest(".kanban-column");if(a.classList.remove("is-touch-dragging"),t.querySelectorAll(".kanban-column").forEach(L=>L.classList.remove("is-over")),v&&v.dataset.status){let L=v.dataset.status;await Le(H,L)}H=null}})}),t.querySelectorAll(".kanban-column").forEach(a=>{a.addEventListener("dragover",n=>{n.preventDefault(),a.classList.add("is-over")}),a.addEventListener("dragleave",()=>{a.classList.remove("is-over")}),a.addEventListener("drop",async n=>{if(n.preventDefault(),a.classList.remove("is-over"),!H)return;let d=a.dataset.status;await Le(H,d)})})}function pe(t){de=t,document.querySelector("#bardo-modal-backdrop")?.remove();let r=t.mode==="edit",e=t.task||{},a=Array.isArray(x?.columns)&&x.columns.length>0?x.columns:ae,n=t.status||e.status||a[0]?.id||"backlog",d=e.priority||"medium";j=ie(e.labels||[]);let l=Ae(x?.tasks||[]),w=Ie(x?.tasks||[]),m=document.createElement("div");m.id="bardo-modal-backdrop",m.className="kanban-modal-backdrop",m.innerHTML=`
    <div class="kanban-modal" role="dialog" aria-modal="true" aria-labelledby="modal-heading">
      <header class="modal-header">
        <h2 id="modal-heading" class="modal-title">${r?"Editar tarea":"Nueva tarea"}</h2>
      ...
      <form id="modal-task-form" class="modal-form">
          ...
          <div class="form-group">
            ...
            <div class="custom-select-wrap">
              <select id="task-status-input" class="form-select">
                ${a.map(b=>`<option value="${b.id}" ${b.id===n?"selected":""}>${s(b.label)}</option>`).join("")}
              </select>
              <span class="select-arrow" aria-hidden="true">
                <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"/></svg>
              </span>
        ...
        <footer class="modal-actions">
          <div>
            ${r?'<button id="btn-delete-task" class="btn-danger" type="button">Eliminar tarea</button>':""}
          </div>
          <div class="modal-actions-right">
            ...
            <button id="btn-modal-submit" class="btn-primary" type="submit">
              ${r?"Guardar cambios":"Crear tarea"}
            </button>
          </div>
        </footer>
      </form>
    </div>
  `,document.body.appendChild(m);let v=m.querySelector("#task-chips-wrapper"),L=m.querySelector("#task-chips-box"),u=m.querySelector("#task-chips-selected"),k=m.querySelector("#task-chip-input"),E=m.querySelector("#task-chip-dropdown");function T(){u&&(u.innerHTML=j.map((b,h)=>{let I=b.color||W(b.name);return`
      ...
      `}).join(""),u.querySelectorAll("[data-remove-chip-idx]").forEach(b=>{b.addEventListener("click",h=>{h.stopPropagation();let I=Number(b.dataset.removeChipIdx);j.splice(I,1),T(),k&&(k.placeholder=j.length?"Otro chip\u2026":"Escribe o crea un chip\u2026")})}))}function B(b=""){if(!E)return;let h=b.trim(),I=new Set(j.map(y=>y.name.toLowerCase())),q=l.filter(y=>!I.has(y.name.toLowerCase())&&(!h||y.name.toLowerCase().includes(h.toLowerCase()))),M="",S=l.some(y=>y.name.toLowerCase()===h.toLowerCase())||j.some(y=>y.name.toLowerCase()===h.toLowerCase());if(h&&!S)if(l.length>=8)M+=`
      ...
      `}if(!M){E.style.display="none";return}E.innerHTML=M,E.style.display="flex",E.querySelectorAll("[data-create-chip]").forEach(y=>{y.addEventListener("mousedown",o=>{o.preventDefault(),o.stopPropagation();let c=y.dataset.createChip,i=y.dataset.chipColor;j.push({name:c,color:i}),T(),k&&(k.value="",k.placeholder="Otro chip\u2026",k.focus()),E.style.display="none"})}),E.querySelectorAll("[data-pick-chip]").forEach(y=>{y.addEventListener("mousedown",o=>{o.preventDefault(),o.stopPropagation();let c=y.dataset.pickChip,i=y.dataset.chipColor;j.push({name:c,color:i}),T(),k&&(k.value="",k.placeholder="Otro chip\u2026",k.focus()),E.style.display="none"})})}L?.addEventListener("click",()=>{k?.focus()}),k?.addEventListener("focus",()=>{B(k.value)}),k?.addEventListener("input",b=>{B(b.target.value)}),k?.addEventListener("keydown",b=>{if(b.key==="Enter"||b.key===","){b.preventDefault();let h=k.value.trim().replace(/^,|,$/g,"").slice(0,24);if(!h)return;if(!j.some(q=>q.name.toLowerCase()===h.toLowerCase())){let q=l.find(S=>S.name.toLowerCase()===h.toLowerCase());if(!q&&l.length>=8){N(`M\xE1ximo ${8} chips por tablero`,"info");return}let M=q?.color||W(h);j.push({name:h,color:M}),T()}k.value="",k.placeholder="Otro chip\u2026",E&&(E.style.display="none")}else b.key==="Backspace"&&!k.value&&j.length&&(j.pop(),T(),k.placeholder=j.length?"Otro chip\u2026":"Escribe o crea un chip\u2026",B(""))}),T();let oe=m.querySelector("#discord-member-box"),A=m.querySelector("#task-assignee-name-input"),U=m.querySelector("#task-assignee-id-input"),D=m.querySelector("#discord-member-dropdown"),ne=m.querySelector("#btn-clear-assignee");function Z(b=""){if(!D)return;let h=b.trim().replace(/^@/,"").toLowerCase(),I=Ee(x),q=I;h&&(q=I.filter(S=>{let y=(S.name||"").toLowerCase(),o=(S.username||"").toLowerCase(),c=String(S.id||"");return y.includes(h)||o.includes(h)||c.includes(h)}));let M="";if(q.length>0)for(let S of q){let y=getMemberRoleBadge(S);M+=`
        ...
        `}else h?M=`<div style="padding: 10px 12px; font-size: 12px; color: var(--kb-text-dim); text-align: center;">No se encontraron miembros que coincidan con "${s(h)}"</div>`:M='<div style="padding: 10px 12px; font-size: 12px; color: var(--kb-text-dim); text-align: center;">Escribe un nombre o selecciona un miembro</div>';if(!M){D.style.display="none";return}D.innerHTML=M,D.style.display="flex",setTimeout(()=>{D.scrollIntoView({block:"nearest",behavior:"smooth"})},40),D.querySelectorAll(".member-menu-item").forEach(S=>{S.addEventListener("mousedown",y=>{y.preventDefault(),y.stopPropagation();let o=S.dataset.memberId,c=S.dataset.memberName,i=S.dataset.memberUsername,g=S.dataset.memberAvatar;if(A&&(A.value=c),U&&(U.value=o),D.style.display="none",c&&x){let f=Array.isArray(x.members)?[...x.members]:[];f.some(p=>p.name.toLowerCase()===c.toLowerCase()||o&&String(p.id)===String(o))||(f.push({id:o,name:c,username:i||"",avatarUrl:g||null}),x.members=f,he({members:f}).catch(()=>{}))}})})}A?.addEventListener("focus",()=>{Z(A.value)}),A?.addEventListener("input",b=>{U&&!/^\d{17,20}$/.test(b.target.value.trim())&&(U.value=""),Z(b.target.value)}),ne?.addEventListener("click",b=>{b.stopPropagation(),A&&(A.value=""),U&&(U.value=""),ne.remove()}),m.addEventListener("click",b=>{!v?.contains(b.target)&&E&&(E.style.display="none"),!oe?.contains(b.target)&&D&&(D.style.display="none"),b.target===m&&ee()});let F=m.querySelector("#task-priority-input");m.querySelectorAll("#priority-selector .seg-btn").forEach(b=>{b.addEventListener("click",()=>{m.querySelectorAll("#priority-selector .seg-btn").forEach(h=>h.classList.remove("is-selected")),b.classList.add("is-selected"),F.value=b.dataset.priority})});function ee(){de=null,m.remove()}if(m.querySelector("#btn-modal-close")?.addEventListener("click",ee),m.querySelector("#btn-modal-cancel")?.addEventListener("click",ee),window.addEventListener("keydown",function b(h){h.key==="Escape"&&de&&(ee(),window.removeEventListener("keydown",b))}),r){let b=m.querySelector("#btn-delete-task"),h=!1;b?.addEventListener("click",async()=>{if(!h){h=!0,b.textContent="\xBFConfirmar eliminaci\xF3n?",b.classList.add("is-confirming"),setTimeout(()=>{h=!1,b.textContent="Eliminar tarea",b.classList.remove("is-confirming")},3500);return}try{await We(e.id),ee(),N("Tarea eliminada","success"),await V(!1)}catch(I){console.error("Error eliminando tarea:",I),N("No se pudo eliminar la tarea","error")}})}m.querySelector("#modal-task-form")?.addEventListener("submit",async b=>{b.preventDefault();let h=m.querySelector("#task-title-input")?.value.trim();if(!h)return;let I=m.querySelector("#task-desc-input")?.value.trim()||"",q=m.querySelector("#task-status-input")?.value||a[0]?.id||"backlog",M=m.querySelector("#task-priority-input")?.value||"medium",S=A?.value.trim()||null,y=U?.value.trim()||null;if(S){if(/^\d{17,20}$/.test(S))y=S;else if(!y||!/^\d{17,20}$/.test(y)){let C=[...x?.members||[],...G].find(p=>p.name&&p.name.toLowerCase()===S.toLowerCase()||p.username&&p.username.toLowerCase()===S.toLowerCase().replace(/^@/,"")||p.id&&p.id===S);C&&C.id&&/^\d{17,20}$/.test(String(C.id))&&(y=String(C.id))}}let o=y||null,c=S||null,i=j,g=m.querySelector("#btn-modal-submit");g&&(g.disabled=!0);try{if(r?(await ze(e.id,{title:h,description:I,status:q,priority:M,assigneeId:o,assigneeName:c,labels:i}),N("Cambios guardados","success")):(await Xe({title:h,description:I,status:q,priority:M,assigneeId:o,assigneeName:c,labels:i}),N("Tarea creada","success")),c){let f=Array.isArray(x?.members)?[...x.members]:[];f.some(p=>p.name.toLowerCase()===c.toLowerCase()||o&&p.id===o)||(f.push({id:o||`m-${Date.now()}`,name:c,username:c}),x.members=f,he({members:f}).catch(()=>{}))}ee(),await V(!1)}catch(f){console.error("Error guardando tarea:",f),N(f.message||"No se pudo guardar la tarea","error"),g&&(g.disabled=!1)}})}function Se(t=null){let r=!!t,e=x?.columns||ae,a=t?.label||"",n=t?.color||O[e.length%O.length].color,d=document.createElement("div");d.id="bardo-column-modal-backdrop",d.className="kanban-modal-backdrop",d.innerHTML=`
    <div class="kanban-modal" role="dialog" aria-modal="true">
      <header class="modal-header">
        <h2 class="modal-title">${r?"Editar columna":`Nueva columna (m\xE1x. ${5})`}</h2>
      ...
      <form id="col-modal-form" class="modal-form">
        <div class="form-group">
          ...
          <input id="col-name-input" class="form-input" type="text" placeholder="Ej: En revisi\xF3n" value="${s(a)}" required maxlength="30" autofocus />
        </div>
        <div class="form-group">
        ...
        <footer class="modal-actions">
          <div>
            ${r&&e.length>1?'<button id="btn-delete-col" class="btn-danger" type="button">Eliminar columna</button>':""}
          </div>
          <div class="modal-actions-right">
            <button id="btn-col-modal-cancel" class="btn-secondary" type="button">Cancelar</button>
            <button id="btn-col-modal-submit" class="btn-primary" type="submit">${r?"Guardar":"Crear columna"}</button>
          </div>
        </footer>
      </form>
    </div>
  `,document.body.appendChild(d);let l=d.querySelector("#col-color-input");d.querySelectorAll(".color-dot-btn").forEach(v=>{v.addEventListener("click",()=>{d.querySelectorAll(".color-dot-btn").forEach(L=>{L.classList.remove("is-selected"),L.style.outline="none"}),v.classList.add("is-selected"),v.style.outline="2px solid #fff",v.style.outlineOffset="2px",l.value=v.dataset.color})});function w(){d.remove()}if(d.querySelector("#btn-col-modal-close")?.addEventListener("click",w),d.querySelector("#btn-col-modal-cancel")?.addEventListener("click",w),d.addEventListener("click",v=>{v.target===d&&w()}),r&&e.length>1){let v=d.querySelector("#btn-delete-col"),L=!1;v?.addEventListener("click",async()=>{if(!L){L=!0,v.textContent="\xBFConfirmar eliminaci\xF3n?",v.classList.add("is-confirming"),setTimeout(()=>{L=!1,v.textContent="Eliminar columna",v.classList.remove("is-confirming")},3500);return}let u=e.filter(k=>k.id!==t.id);try{await $e(u),x.columns=u,w(),N("Columna eliminada","success"),await V(!1)}catch(k){console.error("Error eliminando columna:",k),N(k.message||"No se pudo eliminar la columna","error")}})}d.querySelector("#col-modal-form")?.addEventListener("submit",async v=>{v.preventDefault();let L=d.querySelector("#col-name-input")?.value.trim();if(!L)return;let u=l?.value||"#5865f2",k;if(r)k=e.map(T=>T.id===t.id?{...T,label:L,color:u}:T);else{if(e.length>=5){N(`M\xE1ximo ${5} columnas por tablero`,"error");return}let T=L.toLowerCase().replace(/[^a-z0-9_-]/g,"")||`col-${Date.now()}`,B=T,oe=1;for(;e.some(A=>A.id===B);)B=`${T}-${oe}`,oe+=1;k=[...e,{id:B,label:L,color:u}]}let E=d.querySelector("#btn-col-modal-submit");E&&(E.disabled=!0);try{await $e(k),x.columns=k,w(),N(r?"Columna guardada":"Columna creada","success"),await V(!1)}catch(T){console.error("Error guardando columna:",T),N(T.message||"No se pudo guardar la columna","error"),E&&(E.disabled=!1)}})}async function Ge(t){if(!t)return;let e=[...Array.isArray(t.members)?[...t.members]:[]],n=[...Array.isArray(t.columns)&&t.columns.length>0?JSON.parse(JSON.stringify(t.columns)):JSON.parse(JSON.stringify(ae))];J&&await Ce(J);let d=Ie(t.tasks||[]),l=document.createElement("div");l.id="bardo-board-settings-modal-backdrop",l.className="kanban-modal-backdrop",l.innerHTML=`
    <div class="kanban-modal" role="dialog" aria-modal="true" style="max-width: 560px;">
      ...
      <form id="board-settings-form" class="modal-form">
        <div class="form-group">
          ...
          <input id="board-name-input" class="form-input" type="text" placeholder="Ej: Proyecto Alfa" value="${s(t.name)}" required maxlength="80" autofocus />
        </div>

        <div class="form-group">
          <label for="board-desc-input">Descripci\xF3n</label>
          <textarea id="board-desc-input" class="form-textarea" placeholder="Prop\xF3sito, equipo o alcance de este tablero\u2026" maxlength="500">${s(t.description||"")}</textarea>
        </div>
  ...
  `,document.body.appendChild(l);let w=l.querySelector("#board-modal-columns-list"),m=l.querySelector("#board-col-count-text"),v=l.querySelector("#btn-show-add-col-box"),L=l.querySelector("#board-add-col-box"),u=l.querySelector("#new-col-name-input"),k=l.querySelector("#btn-confirm-add-col"),E=l.querySelector("#btn-cancel-add-col"),T=l.querySelector("#new-col-palette"),B=O[0].color;function oe(){T&&(T.innerHTML=O.map(o=>`
    ...
    `).join(""),T.querySelectorAll("[data-pick-new-col-color]").forEach(o=>{o.addEventListener("click",()=>{B=o.dataset.pickNewColColor,oe()})}))}let A=null;function U(){w&&(m&&(m.textContent=`${n.length}/${5}`),v&&(v.style.display=n.length<5?"inline-flex":"none"),w.innerHTML=n.map((o,c)=>`
    ...
    `).join(""),w.querySelectorAll("[data-col-input-idx]").forEach(o=>{o.addEventListener("input",c=>{let i=Number(c.target.dataset.colInputIdx);n[i]&&(n[i].label=c.target.value)})}),w.querySelectorAll(".modal-column-row").forEach(o=>{o.addEventListener("dragstart",i=>{A=Number(o.dataset.colIdx),o.classList.add("is-dragging"),i.dataTransfer.effectAllowed="move",i.dataTransfer.setData("text/plain",String(A))}),o.addEventListener("dragend",()=>{A=null,o.classList.remove("is-dragging"),w.querySelectorAll(".modal-column-row").forEach(i=>i.classList.remove("is-drag-over"))}),o.addEventListener("dragover",i=>{i.preventDefault(),i.dataTransfer.dropEffect="move";let g=Number(o.dataset.colIdx);A!==null&&A!==g&&o.classList.add("is-drag-over")}),o.addEventListener("dragleave",()=>{o.classList.remove("is-drag-over")}),o.addEventListener("drop",i=>{i.preventDefault(),o.classList.remove("is-drag-over");let g=Number(o.dataset.colIdx);if(A!==null&&A!==g){let[f]=n.splice(A,1);n.splice(g,0,f),U()}});let c=o.querySelector(".modal-column-drag-handle");c&&(c.addEventListener("touchstart",()=>{A=Number(o.dataset.colIdx),o.classList.add("is-dragging")},{passive:!0}),c.addEventListener("touchmove",i=>{if(A===null)return;let g=i.touches[0],C=document.elementFromPoint(g.clientX,g.clientY)?.closest(".modal-column-row");w.querySelectorAll(".modal-column-row").forEach(p=>{p===C&&Number(p.dataset.colIdx)!==A?p.classList.add("is-drag-over"):p.classList.remove("is-drag-over")})},{passive:!0}),c.addEventListener("touchend",i=>{if(A!==null){let g=i.changedTouches[0],C=document.elementFromPoint(g.clientX,g.clientY)?.closest(".modal-column-row");if(C){let p=Number(C.dataset.colIdx);if(!isNaN(p)&&p!==A){let[P]=n.splice(A,1);n.splice(p,0,P)}}A=null,U()}}))}),w.querySelectorAll("[data-remove-col-idx]").forEach(o=>{o.addEventListener("click",()=>{let c=Number(o.dataset.removeColIdx);n.length>1&&(n.splice(c,1),U())})}))}v?.addEventListener("click",()=>{L&&(L.style.display="block"),v&&(v.style.display="none"),u&&(u.value="",u.focus()),B=O[n.length%O.length].color,oe()}),E?.addEventListener("click",()=>{L&&(L.style.display="none"),v&&(v.style.display=n.length<5?"inline-flex":"none")});function D(){let o=u?.value.trim();if(!o)return;if(n.length>=5){N(`M\xE1ximo ${5} columnas por tablero`,"error");return}let c=o.toLowerCase().replace(/[^a-z0-9_-]/g,"")||`col-${Date.now()}`,i=c,g=1;for(;n.some(f=>f.id===i);)i=`${c}-${g}`,g+=1;n.push({id:i,label:o,color:B}),L&&(L.style.display="none"),U()}k?.addEventListener("click",D),u?.addEventListener("keydown",o=>{o.key==="Enter"&&(o.preventDefault(),D())});let ne=l.querySelector("#board-members-list"),Z=l.querySelector("#board-member-suggestions"),F=l.querySelector("#board-member-add-input"),ee=l.querySelector("#btn-add-member-manual"),K=l.querySelector("#board-member-dropdown");function b(o){if(!o||!Array.isArray(o.roles)||!Array.isArray(te))return null;for(let c of te)if(o.roles.includes(c.id))return c;return null}function h(){if(ne){if(e.length===0){ne.innerHTML='<span class="form-helper-text">No hay miembros configurados. Se sugerir\xE1n los miembros del servidor.</span>';return}ne.innerHTML=e.map((o,c)=>{let i=G.find(f=>String(f.id)===String(o.id))||o,g=b(i);return`
    ...
    `).join(""),c.querySelectorAll("[data-role-id]").forEach(f=>{f.addEventListener("click",()=>{let C=f.dataset.roleId,p=g.find(Y=>Y.role.id===C);if(!p||p.unaddedCount===0)return;let P=0;for(let Y of p.members)i.has(String(Y.id).toLowerCase())||(e.push({id:Y.id,name:Y.name,username:Y.username||"",avatarUrl:Y.avatarUrl||null}),i.add(String(Y.id).toLowerCase()),P+=1);h(),I(),q(),N(`A\xF1adidos ${P} miembros con el rol @${p.role.name}`,"success")})})}function M(o){let c=String(o||"").trim();if(!c)return;let i=c.replace(/^@/,""),g=/^\d{17,20}$/.test(i);e.find(C=>C.name.toLowerCase()===i.toLowerCase()||C.id&&C.id===i)||(e.push({id:g?i:`m-${Date.now()}`,name:i,username:g?"":i}),h(),I(),q()),F&&(F.value=""),K&&(K.style.display="none")}ee?.addEventListener("click",()=>{M(F?.value)}),F?.addEventListener("keydown",o=>{o.key==="Enter"&&(o.preventDefault(),M(F.value))}),F?.addEventListener("input",o=>{let c=o.target.value.trim();if(!c||!K){K&&(K.style.display="none");return}let i=c.toLowerCase().replace(/^@/,""),g=new Set(e.map(p=>String(p.id||p.name).toLowerCase())),f=G.filter(p=>!g.has(String(p.id).toLowerCase())&&!g.has(p.name.toLowerCase())&&(p.name.toLowerCase().includes(i)||p.username&&p.username.toLowerCase().includes(i))),C="";f.length>0?C+=f.map(p=>`
      ...
      `).join(""):c&&(C='<div style="padding: 10px 12px; font-size: 12px; color: var(--kb-text-dim); text-align: center;">No se encontraron miembros del servidor que coincidan</div>'),K.innerHTML=C,K.style.display="flex",K.querySelectorAll("[data-pick-id]").forEach(p=>{p.addEventListener("click",()=>{let P=p.dataset.pickId,Y=p.dataset.pickName,Me=p.dataset.pickUsername,Ne=p.dataset.pickAvatar||null;e.push({id:P,name:Y,username:Me,avatarUrl:Ne}),h(),I(),q(),F&&(F.value=""),K.style.display="none"})})});function S(){l.remove()}if(l.querySelector("#btn-board-modal-close")?.addEventListener("click",S),l.querySelector("#btn-board-modal-cancel")?.addEventListener("click",S),l.addEventListener("click",o=>{o.target===l&&S()}),U(),h(),I(),q(),(G.length===0||te.length===0)&&Q){let o=J?.guildId?`?guild_id=${encodeURIComponent(J.guildId)}`:"";Promise.all([fetch(`/api/boards/${encodeURIComponent(Q)}/guild-members${o}`,{headers:{Accept:"application/json"}}).then(c=>c.json()).catch(()=>null),fetch(`/api/boards/${encodeURIComponent(Q)}/guild-roles${o}`,{headers:{Accept:"application/json"}}).then(c=>c.json()).catch(()=>null)]).then(([c,i])=>{Array.isArray(c?.members)&&c.members.length>0&&(G=c.members),Array.isArray(i?.roles)&&i.roles.length>0&&(te=i.roles),h(),I(),q()}).catch(()=>{})}l.querySelector("#board-settings-form")?.addEventListener("submit",async o=>{o.preventDefault();let c=l.querySelector("#board-name-input")?.value.trim();if(!c)return;let i=l.querySelector("#board-desc-input")?.value.trim()||"",g=l.querySelector("#btn-board-modal-submit");g&&(g.disabled=!0);try{let f=await he({name:c,description:i,columns:n,members:e});x&&(x={...x,name:c,description:i,members:e,columns:n}),S(),N("Tablero actualizado","success"),R(document.querySelector("#kanban-content"),x)}catch(f){console.error("Error guardando configuraci\xF3n del tablero:",f),N(f.message||"No se pudo guardar la configuraci\xF3n","error"),g&&(g.disabled=!1)}})}async function qe(){let t=new URLSearchParams;J?.guildId&&t.set("guild_id",J.guildId);let r=t.toString()?`?${t.toString()}`:"",e=await fetch(`/api/boards/${encodeURIComponent(Q)}${r}`,{headers:{Accept:"application/json"},cache:"no-store"});if(!e.ok)throw new Error(`HTTP ${e.status}`);let a=await e.json();return Array.isArray(a?.guildMembers)&&a.guildMembers.length>0&&(G=a.guildMembers),Array.isArray(a?.guildRoles)&&a.guildRoles.length>0&&(te=a.guildRoles),a}async function he(t){if(!_)throw new Error("Se requiere contexto de Activity");let r=await fetch(`/api/boards/${encodeURIComponent(Q)}`,{method:"PATCH",headers:{"Content-Type":"application/json","x-bardo-instance-id":_},body:JSON.stringify(t)});if(!r.ok){let e=await r.json().catch(()=>({}));throw new Error(e?.error||`Error al actualizar tablero (HTTP ${r.status})`)}return r.json()}async function $e(t){if(!_)throw new Error("Se requiere contexto de Activity");let r=await fetch(`/api/boards/${encodeURIComponent(Q)}/columns`,{method:"PATCH",headers:{"Content-Type":"application/json","x-bardo-instance-id":_},body:JSON.stringify({columns:t})});if(!r.ok)throw new Error(`Error al guardar columnas (HTTP ${r.status})`);return r.json()}async function Xe(t){if(!_)throw new Error("Se requiere contexto de Activity");let r=await fetch(`/api/boards/${encodeURIComponent(Q)}/tasks`,{method:"POST",headers:{"Content-Type":"application/json","x-bardo-instance-id":_},body:JSON.stringify(t)});if(!r.ok)throw new Error(`Error al crear tarea (HTTP ${r.status})`);return r.json()}async function ze(t,r){if(!_)throw new Error("Se requiere contexto de Activity");let e=await fetch(`/api/tasks/${encodeURIComponent(t)}`,{method:"PATCH",headers:{"Content-Type":"application/json","x-bardo-instance-id":_},body:JSON.stringify(r)});if(!e.ok)throw new Error(`Error al actualizar tarea (HTTP ${e.status})`);return e.json()}async function We(t){if(!_)throw new Error("Se requiere contexto de Activity");let r=await fetch(`/api/tasks/${encodeURIComponent(t)}`,{method:"DELETE",headers:{"x-bardo-instance-id":_}});if(!r.ok)throw new Error(`Error al borrar tarea (HTTP ${r.status})`);return r.json()}async function Le(t,r){if(!_){N("Sesi\xF3n de Activity no identificada","error");return}if(x?.tasks){let e=x.tasks.find(a=>a.id===t);e&&(e.status=r,R(document.querySelector("#kanban-content"),x))}try{await ze(t,{status:r}),await V(!1)}catch(e){console.error("Error al mover tarea:",e),N("No se pudo mover la tarea","error"),await V(!1)}}async function V(t=!1){if(me||!Q)return;me=!0;let r=document.querySelector("#btn-sync"),e=document.querySelector("#sync-indicator");t&&r&&r.classList.add("is-spinning"),e&&(e.textContent="Sincronizando\u2026");try{let a=await qe();x=a,!de&&!H&&R(document.querySelector("#kanban-content"),a),e&&(e.textContent="Actualizado")}catch(a){console.error("Error sincronizando tablero:",a),t&&N("Error al conectar con Bardo","error"),e&&(e.textContent="Sin conexi\xF3n")}finally{me=!1,r&&r.classList.remove("is-spinning")}}function Je(){be&&clearInterval(be),be=setInterval(()=>{document.visibilityState==="visible"&&!de&&!H&&V(!1)},7500),document.addEventListener("visibilitychange",()=>{document.visibilityState==="visible"&&V(!1)})}async function Qe(){let t=await De();if(!t)return;Q=t;let r=Pe();try{let e=await qe();R(r,e),Je()}catch(e){console.error("No se pudo abrir el tablero:",e),r.className="kanban-state",r.innerHTML=`
