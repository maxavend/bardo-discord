...
function fallbackToDefault(schema) {
  const transform2 = custom().transform((data2) => {
    const res = schema.safeParse(data2);
    if (res.success) {
      return res.data;
    }
...
function parseEventPayload(data2) {
  const event = data2.evt;
  if (!(event in Events)) {
    throw new Error(`Unrecognized event type ${data2.evt}`);
  }
  const eventSchema = EventSchema[event];
  return eventSchema.payload.parse(data2);
}
var EventSchema = {
  /**
   * @description
...
function parseResponseData({ cmd, data: data2 }) {
  switch (cmd) {
    case Commands.AUTHORIZE:
      return AuthorizeResponse.parse(data2);
    case Commands.CAPTURE_SHORTCUT:
      return CaptureShortcutResponse.parse(data2);
    case Commands.ENCOURAGE_HW_ACCELERATION:
      return EncourageHardwareAccelerationResponse.parse(data2);
    case Commands.GET_CHANNEL:
      return GetChannelResponse.parse(data2);
    case Commands.GET_CHANNELS:
      return GetChannelsResponse.parse(data2);
    case Commands.GET_CHANNEL_PERMISSIONS:
      return GetChannelPermissionsResponse.parse(data2);
    case Commands.GET_GUILD:
      return GetGuildResponse.parse(data2);
    case Commands.GET_GUILDS:
      return GetGuildsResponse.parse(data2);
    case Commands.GET_PLATFORM_BEHAVIORS:
      return GetPlatformBehaviorsResponse.parse(data2);
    case Commands.GET_CHANNEL:
      return GetChannelResponse.parse(data2);
    case Commands.SELECT_TEXT_CHANNEL:
      return SelectTextChannelResponse.parse(data2);
    case Commands.SELECT_VOICE_CHANNEL:
      return SelectVoiceChannelResponse.parse(data2);
    case Commands.SET_ACTIVITY:
      return SetActivityResponse.parse(data2);
    case Commands.GET_SKUS_EMBEDDED:
      return GetSkusResponse.parse(data2);
    case Commands.GET_ENTITLEMENTS_EMBEDDED:
      return GetEntitlementsResponse.parse(data2);
    case Commands.SET_CONFIG:
      return SetConfigResponse.parse(data2);
    case Commands.START_PURCHASE:
      return StartPurchaseResponse.parse(data2);
    case Commands.SUBSCRIBE:
    case Commands.UNSUBSCRIBE:
      return SubscribeResponse.parse(data2);
    case Commands.USER_SETTINGS_GET_LOCALE:
      return UserSettingsGetLocaleResponse.parse(data2);
    case Commands.OPEN_EXTERNAL_LINK:
      return OpenExternalLinkResponse.parse(data2);
    // Empty Responses
    ...
    case Commands.GET_ENTITLEMENTS:
      return EmptyResponse.parse(data2);
...
var DiscordSDK = class {
  ...
  constructor(clientId, configuration) {
    ...
    this.handleMessage = (event) => {
      ...
      const [opcode, data2] = tuple;
      switch (opcode) {
        case Opcodes.HELLO:
          return;
        case Opcodes.CLOSE:
          return this.handleClose(data2);
        case Opcodes.HANDSHAKE:
          return this.handleHandshake();
        case Opcodes.FRAME:
          return this.handleFrame(data2);
        default:
    ...
    const instanceId2 = urlParams.get("instance_id");
    if (!instanceId2) {
...
async function resolveActivityInstanceId() {
  ...
  try {
    const sdk2 = new DiscordSDK(resolveClientId());
    await sdk2.ready();
    return sdk2.instanceId || null;
  } catch (error) {
...
async function fetchSource(documentId, instanceId2, maxAttempts = 6) {
  let lastStatus = null;
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    const response = await nativeFetch(`/api/documents/${encodeURIComponent(documentId)}/source`, {
      headers: { "x-bardo-instance-id": instanceId2 },
      cache: "no-store"
    });
...
async function cacheNormalization(documentId, instanceId2, markdown) {
  ...
  const response = await nativeFetch(`/api/documents/${encodeURIComponent(documentId)}/normalize`, {
    ...
    headers: {
      ...
      "x-bardo-instance-id": instanceId2
    },
...
async function normalizePendingDocument(data2, documentId) {
  if (data2.importStatus !== "pending" || !data2.hasSource) return data2;
  const instanceId2 = await resolveActivityInstanceId();
  if (!instanceId2) return data2;
  const source = await fetchSource(documentId, instanceId2);
  let markdown;
  if (data2.sourceType === "pdf") {
    markdown = await importPdf(source, data2.title || "Documento");
  } else if (data2.sourceType === "docx") {
    markdown = await importDocx(source, data2.title || "Documento");
  } else {
    return data2;
  }
  await cacheNormalization(documentId, instanceId2, markdown);
  return {
    ...data2,
    markdown,
    importStatus: "ready",
...
function parseDocumentMetadataRequest(input, init2) {
  const method = String(init2?.method || (input instanceof Request ? input.method : "GET")).toUpperCase();
...
window.fetch = async function bardoFetch(input, init2) {
  const documentId = parseDocumentMetadataRequest(input, init2);
  const response = await nativeFetch(input, init2);
  if (!documentId || !response.ok) return response;
  try {
    const data2 = await response.clone().json();
    if (data2?.importStatus !== "pending" || !data2?.hasSource) return response;
    const normalized = await normalizePendingDocument(data2, documentId);
    if (normalized === data2) return response;
...
function initTheme(sdk2) {
  const params = new URLSearchParams(window.location.search);
  const paramTheme = params.get("theme");
  if (paramTheme) {
    applyTheme(paramTheme);
  } else if (sdk2?.theme) {
    applyTheme(sdk2.theme);
  } else if (sdk2?.config?.theme) {
    applyTheme(sdk2.config.theme);
  ...
  if (sdk2?.subscribe) {
    try {
      sdk2.subscribe("THEME_CHANGE", ({ theme }) => {
...
async function fetchActivityContextWithRetry(instanceId2, maxRetries = 5) {
  if (!instanceId2) return null;
  for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
    try {
      const response = await fetch(`/api/activity-context/${encodeURIComponent(instanceId2)}`, {
        headers: { Accept: "application/json" },
        cache: "no-store"
      });
      if (response.ok) {
        const data2 = await response.json();
        if (data2 && data2.documentId) {
          return data2.documentId;
        }
      }
    } catch (err) {
...
function renderDocument(data2) {
  currentDocumentData = data2;
  titleEl.textContent = data2.title || "Documento";
  document.title = `${data2.title || "Documento"} \xB7 Bardo`;
  const meta = [];
  const date = formatDate(data2.createdAt);
  if (date) meta.push(`<span>${escapeHtml(date)}</span>`);
  metaEl.innerHTML = meta.join("");
  const markdown = stripLeadingTitle(data2.markdown || "", data2.title || "");
...
async function start() {
  ...
  for (const candidate of candidates) {
    try {
      const data2 = await fetchDocument(candidate.id);
      if (data2) {
        renderDocument(data2);
        return;
      }
...
function initTheme2(sdk2) {
  const params = new URLSearchParams(window.location.search);
  const paramTheme = params.get("theme");
  if (paramTheme) {
    applyTheme2(paramTheme);
  } else if (sdk2?.theme) {
    applyTheme2(sdk2.theme);
  } else if (sdk2?.config?.theme) {
    applyTheme2(sdk2.config.theme);
  } else if (window.matchMedia && window.matchMedia("(prefers-color-scheme: light)").matches) {
    applyTheme2("light");
  } else {
    applyTheme2("dark");
...
async function refreshDiscordParticipants(sdk2) {
  if (!sdk2) return;
  try {
    if (sdk2.commands?.getInstanceConnectedParticipants) {
      const res = await sdk2.commands.getInstanceConnectedParticipants();
  ...
  try {
    if (sdk2.commands?.getChannel && sdk2.channelId) {
      const ch = await sdk2.commands.getChannel({ channel_id: sdk2.channelId });
...
async function initDiscordSdk2() {
  ...
  try {
    const sdk2 = new DiscordSDK(resolveClientId3());
    await sdk2.ready();
    activeDiscordSdk2 = sdk2;
    initTheme2(sdk2);
    try {
      if (sdk2.subscribe) {
        sdk2.subscribe("ACTIVITY_INSTANCE_PARTICIPANTS_UPDATE", ({ participants }) => {
    ...
    await refreshDiscordParticipants(sdk2);
    return sdk2;
  } catch (error) {
...
async function fetchContext(instanceId2, maxAttempts = 5) {
  if (!instanceId2) return null;
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    const response = await fetch(`/api/activity-context/${encodeURIComponent(instanceId2)}`, {
      cache: "no-store",
...
async function resolveBoardTarget() {
  const params = new URLSearchParams(window.location.search);
  const sdk2 = await initDiscordSdk2();
  currentInstanceId = sdk2?.instanceId || params.get("instance_id") || null;
  const direct = parseBoardId(sdk2?.customId) || parseBoardId(params.get("custom_id")) || params.get("board");
...
function showToast(message, type = "info") {
  ...
  toast2.id = "bardo-toast";
  toast2.className = `kanban-toast toast-${type}`;
  toast2.innerHTML = `
    <span class="toast-icon">${type === "success" ? "\u2713" : type === "error" ? "\u2715" : "\u2139"}</span>
    <span class="toast-msg">${escapeHtml2(message)}</span>
  `;
  document.body.appendChild(toast2);
  requestAnimationFrame(() => toast2.classList.add("is-visible"));
  toastTimer = setTimeout(() => {
    toast2.classList.remove("is-visible");
    setTimeout(() => toast2.remove(), 250);
  }, 3200);
}
...
function createShell() {
  ...
  const avatar2 = document.querySelector(".brand-avatar")?.src || "";
  shell.innerHTML = `
    <header class="kanban-topbar">
      <div class="kanban-brand-group">
        ${avatar2 ? `
          <div class="kanban-avatar-box">
            <img class="kanban-avatar" src="${escapeHtml2(avatar2)}" alt="Bardo" />
          </div>
        ` : ""}
    ...
    if (rawAssigneeName) {
      if (/^\d{17,20}$/.test(rawAssigneeName)) {
        ...
        const match = allKnown.find(
          (m) => m.name && m.name.toLowerCase() === rawAssigneeName.toLowerCase() || m.username && m.username.toLowerCase() === rawAssigneeName.toLowerCase().replace(/^@/, "") || m.id && m.id === rawAssigneeName
        );
...
async function fetchBoard() {
  ...
  if (!response.ok) throw new Error(`HTTP ${response.status}`);
  const data2 = await response.json();
  if (Array.isArray(data2?.guildMembers) && data2.guildMembers.length > 0) {
    serverGuildMembers = data2.guildMembers;
  }
  if (Array.isArray(data2?.guildRoles) && data2.guildRoles.length > 0) {
    serverGuildRoles = data2.guildRoles;
  }
  return data2;
}
async function saveBoardSettingsRequest(payload) {
  ...
  if (!response.ok) {
    ...
    throw new Error(data2?.error || `Error al actualizar tablero (HTTP ${response.status})`);
  }
  return response.json();
}
...
// src/event.js
var BARDO_EVENT_PREFIX = "bardo:event:";
var EVENT_TARGET_PREFIX = "event:";
var EVENT_STATUSES = ["draft", "scheduled", "live", "finished", "cancelled"];
var EVENT_BLOCK_TYPES = ["discussion", "presentation", "review", "break", "decision", "other"];
function parseEventTarget(value) {
...
function normalizeEventStatus(value, fallback = "scheduled") {
  const normalized = String(value || "").trim().toLowerCase();
  return EVENT_STATUSES.includes(normalized) ? normalized : fallback;
}
function normalizeEventBlockType(value, fallback = "discussion") {
  const normalized = String(value || "").trim().toLowerCase();
  return EVENT_BLOCK_TYPES.includes(normalized) ? normalized : fallback;
}
function cleanDuration(value, fallback = 0, max = 24 * 60) {
...
function calculateEventTimeline(event) {
...
function totalEventAgendaMinutes(event) {
...
function eventBlockTypeLabel(type) {
  return {
    ...
    other: "Otro"
  }[normalizeEventBlockType(type)] || "Conversaci\xF3n";
}

// src/activity/event.js
var FALLBACK_CLIENT_ID4 = "1539704001535156254";
var sdk = null;
var instanceId = null;
var eventId = null;
var data = null;
...
function resolveClientId4() {
  const host = window.location.hostname || "";
  return host.match(/^([a-zA-Z0-9_-]+)\.discordsays\.com$/i)?.[1] || FALLBACK_CLIENT_ID4;
}
async function initSdk() {
  ...
  if (!embedded) return null;
  try {
    const client = new DiscordSDK(resolveClientId4());
    await client.ready();
    sdk = client;
...
async function fetchContext2(id, attempts = 5) {
  if (!id) return null;
...
async function resolveEventId() {
  ...
  if (direct) return direct;
  const context = await fetchContext2(instanceId);
...
function injectStyles2() {
  ...
  style.textContent = `
    ...
    .ev-note,.ev-decision,.ev-task-ref{margin-top:7px;padding:8px 10px;border-radius:8px;background:var(--surface2);font-size:13px}.ev-note:before{content:'Nota \xB7 ';color:var(--muted)}.ev-decision{border-left:3px solid var(--green)}.ev-decision:before{content:'Decisi\xF3n \xB7 ';font-weight:700}.ev-task-ref{border-left:3px solid var(--accent)}.ev-task-ref:before{content:'Tarea \xB7 ';font-weight:700}.ev-empty{background:var(--surface);border-radius:12px;padding:34px;text-align:center;color:var(--muted)}.ev-add{width:100%;border:1px dashed var(--line);background:transparent;color:var(--muted);padding:12px;border-radius:10px;cursor:pointer;margin-top:10px}.ev-add:hover{background:var(--surface);color:var(--text)}
...
function avatar(person) {
  ...
  return `<span class="ev-avatar" title="${label}">${person.avatarUrl ? `<img src="${esc(person.avatarUrl)}" alt="">` : esc(initials2(label))}</span>`;
}
function personChip(person) {
  return `<span class="ev-person">${person.avatarUrl ? `<img src="${esc(person.avatarUrl)}" alt="">` : `<span class="dot">${esc(initials2(person.displayName))}</span>`}<span>${esc(person.displayName || person.userId)}</span></span>`;
}
...
function renderHero() {
  ...
  return `<section class="ev-hero">
    <div>
      ...
      <div class="ev-meta"><span>${esc(data.eventDate)} \xB7 ${esc(data.startTime)}</span><span class="ev-badge ${data.status === "live" ? "live" : ""}">${esc(eventStatusLabel(data.status))}</span><span>${formatDuration(agendaMinutes || data.expectedDuration)}</span></div>
    ...
    <div class="ev-actions">
      ...
      ${data.status === "live" ? `<button class="ev-btn primary" data-action="finish">Finalizar</button>` : data.status === "finished" ? `<button class="ev-btn" data-action="publish-minutes">Publicar minuta</button><button class="ev-btn primary" data-action="minutes">Abrir minuta</button>` : `<button class="ev-btn primary" data-action="start">Iniciar reuni\xF3n</button>`}
    </div>
  </section>
  ...
  return `<div class="ev-item" data-item-id="${esc(item.id)}">
    <div class="ev-item-row">
      ...
      <div class="ev-item-main">
        ...
        ${data.status === "live" ? `<div class="ev-live-tools"><button class="ev-btn small" data-action="note" data-block="${esc(block.id)}" data-item="${esc(item.id)}">+ Nota</button><button class="ev-btn small" data-action="decision" data-block="${esc(block.id)}" data-item="${esc(item.id)}">+ Decisi\xF3n</button><button class="ev-btn small" data-action="task" data-block="${esc(block.id)}" data-item="${esc(item.id)}">+ Tarea</button></div>` : ""}
      </div>
      <div class="ev-card-actions"><button class="ev-btn ghost small" data-action="edit-item" data-item="${esc(item.id)}" data-block="${esc(block.id)}">Editar</button><button class="ev-btn ghost small" data-action="delete-item" data-item="${esc(item.id)}">\xD7</button></div>
    </div>
  </div>`;
}
function renderAgenda() {
  const timeline = calculateEventTimeline(data);
  if (!timeline.length) return `<div class="ev-empty"><h2>La agenda est\xE1 vac\xEDa</h2><p>Agrega el primer bloque para empezar a estructurar la sesi\xF3n.</p><button class="ev-btn primary" data-action="add-block">+ A\xF1adir bloque</button></div>`;
  return `<div class="ev-agenda">${timeline.map((block) => `<div class="ev-block" data-block-id="${esc(block.id)}" draggable="true">
    ...
    <div class="ev-card">
      <div class="ev-block-head"><div><div class="ev-block-title">${esc(block.title)}</div><div class="ev-block-meta">${esc(eventBlockTypeLabel(block.type))} \xB7 ${block.durationMinutes} min</div></div><div class="ev-card-actions"><span class="ev-drag" title="Reordenar">\u22EE\u22EE</span><button class="ev-btn ghost small" data-action="edit-block" data-block="${esc(block.id)}">Editar</button><button class="ev-btn ghost small" data-action="delete-block" data-block="${esc(block.id)}">\xD7</button></div></div>
      ...
      <button class="ev-add" data-action="add-item" data-block="${esc(block.id)}">+ A\xF1adir punto</button>
      ${data.status === "live" ? `<div class="ev-live-tools"><button class="ev-btn small" data-action="note" data-block="${esc(block.id)}">+ Nota del bloque</button><button class="ev-btn small" data-action="decision" data-block="${esc(block.id)}">+ Decisi\xF3n</button><button class="ev-btn small" data-action="task" data-block="${esc(block.id)}">+ Tarea</button></div>` : ""}
  ...
  </div>`).join("")}<button class="ev-add" data-action="add-block">+ A\xF1adir bloque</button></div>`;
}
...
function openModal2({ title, body, submit = "Guardar", onSubmit, danger = null }) {
  modalOpen = true;
  ...
  if (danger) bg.querySelector("[data-modal-danger]").onclick = async () => {
    if (await danger.run()) close();
  };
...
function itemModal(blockId, item = null) {
  ...
  openModal2({
    ...
    body: `<div class="ev-field"><label>T\xEDtulo</label><input class="ev-input" name="title" value="${esc(item?.title || "")}" required></div><div class="ev-field"><label>Descripci\xF3n</label><textarea class="ev-textarea" name="description">${esc(item?.description || "")}</textarea></div><div class="ev-field"><label>Duraci\xF3n (min, opcional)</label><input class="ev-input" type="number" min="0" max="480" name="duration" value="${Number(item?.durationMinutes || 0)}"></div><div class="ev-field"><label>Protagonistas / presentan</label>${peopleSelect(item?.speakers || [], "speaker")}</div><div class="ev-field"><label>Links (uno por l\xEDnea; opcional: \u201CFigma | https://\u2026\u201C)</label><textarea class="ev-textarea" name="links" placeholder="Figma | https://figma.com/...">${esc(links)}</textarea></div>`,
...
function captureModal(kind, blockId, itemId) {
  const labels = { note: "Nueva nota", decision: "Registrar decisi\xF3n" };
  openModal2({
    title: labels[kind],
    body: `<div class="ev-field"><label>${kind === "note" ? "Nota" : "Decisi\xF3n"}</label><textarea class="ev-textarea" name="content" autofocus required></textarea></div>`,
    submit: "Guardar",
...
function newEventModal() {
  ...
  openModal2({
    title: "Nuevo evento",
    body: `<div class="ev-field"><label>T\xEDtulo</label><input class="ev-input" name="title" required></div><div class="ev-grid2"><div class="ev-field"><label>Fecha</label><input class="ev-input" type="date" name="date" value="${today}" required></div><div class="ev-field"><label>Hora</label><input class="ev-input" type="time" name="time" value="10:00" required></div></div><div class="ev-field"><label>Duraci\xF3n (min)</label><input class="ev-input" type="number" name="duration" value="60" min="1" max="720"></div><div class="ev-field"><label>Descripci\xF3n</label><textarea class="ev-textarea" name="description"></textarea></div>`,
...
async function action(el) {
  ...
  if (actionName === "delete-block" && confirm("\xBFEliminar este bloque y sus puntos?")) {
    await api(`/blocks/${blockId}`, { method: "DELETE" });
    await loadEvent();
  }
  if (actionName === "delete-item" && confirm("\xBFEliminar este punto?")) {
  ...
  if (actionName === "start") {
    ...
    await loadEvent();
    toast("Reuni\xF3n iniciada");
  }
  if (actionName === "finish") {
    await api("/finish", { method: "POST", body: {} });
    await loadEvent();
    toast("Reuni\xF3n finalizada");
  }
  if (actionName === "share") {
    await api("/publish", { method: "POST", body: {} });
    toast("Agenda publicada en Discord");
  }
  if (actionName === "publish-minutes") {
    await api("/publish-minutes", { method: "POST", body: {} });
    toast("Minuta publicada en Discord");
  }
  if (actionName === "duplicate") {
    const res = await api("/duplicate", { method: "POST", body: {} });
  ...
  if (actionName === "minutes") {
    ...
    url.searchParams.set("document", res.documentId);
    url.searchParams.delete("event");
    url.searchParams.delete("custom_id");
  ...
  if (actionName === "cycle-item") {
    ...
    await api(`/items/${item.id}`, { method: "PATCH", body: { status: next } });
    await loadEvent({ quiet: true });
  }
}
function bindEvents() {
  ...
  document.querySelectorAll("[data-open-event]").forEach((el) => el.onclick = async () => {
    eventId = el.dataset.openEvent;
    currentTab = "agenda";
    await loadEvent();
  });
  document.querySelectorAll('.ev-block[draggable="true"]').forEach((el) => {
    el.addEventListener("dragstart", () => {
      draggedBlockId = el.dataset.blockId;
    ...
    el.addEventListener("drop", async (e) => {
      e.preventDefault();
      const target = el.dataset.blockId;
      if (!draggedBlockId || target === draggedBlockId) return;
      const ids = (data.blocks || []).sort((a, b) => a.position - b.position).map((b) => b.id);
      const from = ids.indexOf(draggedBlockId), to = ids.indexOf(target);
      if (from < 0 || to < 0) return;
      ids.splice(to, 0, ids.splice(from, 1)[0]);
      try {
        await api("/reorder-blocks", { method: "POST", body: { ids } });
        await loadEvent({ quiet: true });
      } catch (err) {
        toast(err.message);
      }
...
function startPolling2() {
  ...
  pollTimer = setInterval(() => {
    if (document.visibilityState === "visible" && !modalOpen && currentTab === "agenda") loadEvent({ quiet: true });
  }, 8e3);
}
async function init() {
  await initSdk();
