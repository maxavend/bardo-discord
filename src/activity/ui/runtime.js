...
const MODE_TO_NAV = { home: 'home', board: 'boards', event: 'agenda', document: 'documents' };
...
function hrefFor(key) {
  ...
  if (key === 'home') { params.set('home','1'); ['document','board','event','task','custom_id'].forEach((name)=>params.delete(name)); return url.toString(); }
  const parameter = keyMap[key];
  if (!parameter || !params.get(parameter)) return key === currentMode() ? location.href : null;
  params.delete('home'); ['document','board','event','custom_id'].forEach((name)=>{ if(name!==parameter) params.delete(name); });
  return url.toString();
}
function buildNav() {
  ...
  for(const [key,label] of NAV_ITEMS){ const href=hrefFor(key); const item=href?document.createElement('a'):document.createElement('button'); item.textContent=label; item.dataset.bardoNav=key;
    if(href)item.href=href; else { item.type='button'; item.setAttribute('aria-disabled','true'); item.title=`Abre un ${label.toLowerCase()} desde Bardo para conservar el contexto`; }
...
document.addEventListener('click',(event)=>{
  const backdrop=event.target instanceof Element ? event.target.closest('.kanban-modal-backdrop,.ev-modal-bg,.bardo-product-modal-backdrop') : null;
  const dialog=dialogFor(event.target) || backdrop?.querySelector('.kanban-modal,.ev-modal,.bardo-product-modal,[role="dialog"]');
...
document.addEventListener('keydown',(event)=>{
  if(event.key!=='Escape')return; const dialogs=[...document.querySelectorAll('.kanban-modal,.ev-modal,.bardo-product-modal,[role="dialog"]')].filter((node)=>node.getClientRects().length); const dialog=dialogs.at(-1); if(dialog&&dirtyDialogs.has(dialog)){ event.preventDefault(); event.stopImmediatePropagation(); warnDirty(dialog,null); }
},true);
