...
test('Phase 6 Activity boot keeps adaptive polling and product modules lazy', () => {
  ...
  assert.ok(main.includes("import './resource-polling.js'"));
  assert.ok(main.includes("await import('./board.js')"));
  assert.ok(main.includes("await import('./event.js')"));
  assert.ok(main.includes("await import('./app.js')"));
  assert.ok(main.includes("await import('./planner-member-directory.js')"));
...
test('Planner people are searched remotely, including the first person in an empty list', () => {
  ...
  assert.ok(picker.includes('/api/member-directory?query='));
  ...
  assert.ok(!picker.includes('if (!firstCheckbox?.name) return'));
  assert.match(p6, /referencedEventPeople/);
  assert.match(p6, /guildMembers/);
  assert.doesNotMatch(p6, /fetchGuildMembers/);
  assert.ok(build.includes('src/activity/planner-member-directory.js'));
});
...
test('notification telemetry records outcomes without recipient or entity ids', () => {
  ...
  assert.ok(helper.includes('notification.delivery'));
...
test('Wrangler preserves Phase 6 behind the conversational entry and keeps staging isolated', () => {
  const config = JSON.parse(read('wrangler.jsonc'));
  const conversationEntry = read('src/conversation-entry.js');
  ...
  assert.equal(config.main, 'src/conversation-entry.js');
  assert.ok(conversationEntry.includes("from './p6-entry.js'"));
  assert.equal(config.ai.binding, 'AI');
  assert.equal(config.env.staging.ai.binding, 'AI');
